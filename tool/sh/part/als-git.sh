...
alias git-v='{
  {
    ${git_chatty:-true} && std_not stdlog_quiet 6 || std_not stdlog_quiet 7;
  } && git_opt=--verbose || {
    { ${git_chatty:-true} && std_not stdlog_quiet 5 || std_not stdlog_quiet 6; } && {
      git_opt=
    } || {
      git_opt=--quiet
    }
...
alias :git:list:ignored='git ls-files --ignored --cached --exclude-standard'

...
args_has_none () # ~ <Argv...> # True if argv is empty or before start of next sequence.
{
  test $# -eq 0 -o "${1-}" = "--"
}

args_has_seq () # ~ <Argv...> # True if argv has sequence, except if the first is empty
{
  ! args_has_none "$@" && fnmatch "* -- *" " $* "
}
